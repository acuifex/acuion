...
void Misc::RenderTab()
{
	...
	const char* openSkinType[] = { "StatTrak", "Non-StatTrak" };
	const char* openSkinRarity[] = { "Consumer grade (White)", "Industrial (Light blue)", "Mil-spec (Darker blue)", "Restricted (Purple)", "Classified (Pinkish purple)", "Covert (Red)", "Contraband (Yellow Orange)"};
	...
	ImGui::NextColumn();
	{
		ImGui::BeginChild(XORSTR("Child2"), ImVec2(0, 0), true);
		{
			...
			ImGui::NextColumn();
			{
				...
				ImGui::Combo(XORSTR("##WEAPONRARITY"), &Settings::ProfileChanger::weaponRarity, openSkinRarity, IM_ARRAYSIZE(openSkinRarity));
